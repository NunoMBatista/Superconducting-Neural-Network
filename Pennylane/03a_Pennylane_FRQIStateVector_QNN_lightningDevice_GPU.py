...
Run as:
    python3 03a_Pennylane_FRQIStateVector_QNN_lightningDevice_GPU.py --data-dir ./Datasets/dataset_250_4by4 --img-size 4 --epochs 20 --batch-size 16 --output-dir ./Code02_epochs20
"""
...
def build_qnode(img_size, num_layers):
    ...
    def qnode(inputs, weights):
        img = np_pl.array(inputs.detach().cpu()).reshape(img_size, img_size)
        state = build_frqi_statevector(img)   # <-- NumPy float32
...
def evaluate_model(model, X, y, device):
    ...
    with torch.no_grad():
        preds = [
            model(torch.tensor(x).float().to(device)).item()
            for x in X
        ]
...
def main():
    ...
    model = HybridFRQI(qnode, weight_shapes, num_obs=num_wires).to(device)
    ...
    print("PennyLane device:", qnode.device)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % eval_interval == 0 or (epoch + 1) == args.epochs:
            acc, prec, rec, f1, auc = evaluate_model(model, X_val, y_val, device)
    ...
    with torch.no_grad():
        preds = []
        for x in X_test:
            preds.append(model(torch.tensor(x).float().to(device)).item())
