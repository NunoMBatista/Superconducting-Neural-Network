...
def run_experiment(config, data, run_dir, max_epochs=5000):
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch + 1) % 100 == 0:
            print(f"    Epoch {epoch+1}/5000 - Loss: {epoch_train_loss:.6f}")
...
def main():
    ...
    for i, config in enumerate(combinations):
        ...
        acc = run_experiment(config, data, run_dir, max_epochs=5000)
        
        summary.append({**config, "test_accuracy": acc, "epochs_run": 5000})
